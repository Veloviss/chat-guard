...
client.on('message', async message => {
  ...
  client.on("messageUpdate", async(oldMessage, newMessage) => { 
      ...
      if(newMessage.content.length > "500") {
        ...
        let botVoiceChannel = client.channels.cache.get("831945255107952691"); //dilerseniz silebilirsiniz
...
client.on("messageDelete", async message => {
   ...
   let aroxLog = message.guild.channels.cache.get("826837744402956328") // Mesaj Silinme Log
  
  if(aroxLog) {
    if (message.author.bot || message.channel.type == "dm") return;
  const arxEmb = new Discord.MessageEmbed().setTimestamp()
    if (message.attachments.first()) {
  aroxLog.send(arxEmb.setAuthor(`${message.author.tag} | Fotoğraf Silindi`, message.author.avatarURL()).setDescription(`<#${message.channel.id}> kanalında <@!${message.author.id}> tarafından bir fotoğraf silindi. 
  
  \`•\` **Fotoğraf İçeriği:** `).setImage(message.attachments.first().proxyURL
  ).setColor("RED").setFooter(`This is velovis!`).setThumbnail(client.user.avatarURL));
      } else {
  aroxLog.send(arxEmb.setAuthor(`${message.author.tag} | Mesaj Silindi`, message.author.avatarURL()).setDescription(`<#${message.channel.id}> kanalında <@!${message.author.id}> tarafından bir mesaj silindi.
  ...
  client.on('messageUpdate', async (oldMessage, newMessage) => {
   ...
   let aroxLog = oldMessage.guild.channels.cache.get("826837745434624050") // Mesaj Silinme Log
  ...
  aroxLog.send(arxEmb.setAuthor(`${oldMessage.author.tag} | Mesaj Düzenlendi`, oldMessage.author.avatarURL()).setDescription(`<#${oldMessage.channel.id}> kanalında <@!${oldMessage.author.id}> tarafından bir mesaj silindi.
  
  \`•\` **Eski Mesaj:** ${oldMessage.content}
  
  \`•\` **Yeni Mesaj:** ${newMessage.content}
  `).setColor("BLACK").setFooter(`This is velovis!`).setThumbnail(client.user.avatarURL)); 
    }}
  );